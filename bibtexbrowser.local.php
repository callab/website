...
define('BIBTEXBROWSER','vef0563951ad96fad3696518df89f88324aaf46ff');
...
// do we add [repo] links ?
@define('BIBTEXBROWSER_REPO_LINKS',true);
...
//     echo $this->getKey();
  ...
  */
  ...
  /** REPO LINK */
  function getRepoLink($iconurl=NULL) {
    $str = $this->getIconOrTxt('repository',$iconurl);
    if ($this->hasField('repository')) {
        return '<a'.(BIBTEXBROWSER_LINKS_IN_NEW_WINDOW?' target="_blank" ':'').' href="'.$this->getField('repository').'">'.$str.'</a>';
    }
    return '';
  }
...
if (BIBTEXBROWSER_REPO_LINKS) {
    $link = $bibentry->getRepoLink();
    if ($link != '') { $links[] = $link; };
  }
...
function DefaultBibliographyStyle(&$bibentry) {
  ...
  if ($type=="inproceedings") {
      $booktitle = __('').' '.'<span itemprop="isPartOf">'.$bibentry->getField(BOOKTITLE).'</span>'; }
...
function splitFullName($author){
    ...
    if (strpos($author,',')===false) {
      ...
      if (substr($author, 0, 6) === '\relax') {
        $lastname = str_replace('\relax ', '', $author);
        $firstname = '';
      }
      else {
...
function poweredby() {
  ...
  $poweredby .= 'Powered by <a href="http://www.monperrus.net/martin/bibtexbrowser/">bibtexbrowser</a><!--vef0563951ad96fad3696518df89f88324aaf46ff-->';
  $poweredby .= '</div>'."\n";
  return $poweredby;
  }
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
   <channel>
      ...
      <generator>bibtexbrowser vef0563951ad96fad3696518df89f88324aaf46ff</generator>
...
class Dispatcher {
  ...
  function diagnosis() {
    ...
    echo "bibtexbrowser version: ef0563951ad96fad3696518df89f88324aaf46ff\n";
